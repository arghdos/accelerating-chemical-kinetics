...
\begin{frame}
 ...
 \begin{itemize}
  ...
  \only<2>
  {
  \item<2-> A SIMD enabled serial CPU solver was \SI{2.0}{\times} and \SI{1.2}{\times}\footnote{Over a serial CPU implementation} faster than the default serial CPU version for single\slash double precision respectively.
  \item<2-> The CBE had the best single precision performance---up to \SI{40.7}\times and \SI{11.5}{\times}\footnotemark[\value{footnote}] speedup for single\slash double precision respectively---due to explicitly managed memory and large on-chip fast memory cache sizes, but was difficult to implement.
  \item<2-> The GPU solver was easy to implement, but difficult to optimize due to small on-chip memory cache sizes; a maximum speedup of \SI{13.7}{\times} and \SI{13.3}{\times}\footnotemark[\value{footnote}] for single\slash double precision (respectively) was achieved.
  }
 \end{itemize}
\end{frame}
...
\begin{itemize}
 \item Automatically generated explicit SIMD-code generated good speedups for atmospheric chemical kinetics~\footfullcite{Linford:2009}~\footfullcite{Linford:2011}
 \begin{itemize}
  \item It is noted that the reported speedups were against a serial reference code
  \item The Cell-Broadband engine had very large speedups, but was difficult to implement (and is no longer produced)
 \end{itemize}
 \item A method to mitigate thread-divergence~\footfullcite{Murray:2012} was developed, resulting in a $\SI{10}{\percent}$ improvemnt
...
\begin{frame}
 ...
 \begin{itemize}
  ...
  \item More modern work~\footfullcite{Novati2008} developed Jacobian and LU factorization update schemes
 \end{itemize}
 Two-step methods avoid order-reduction for very stiff problems~\footfullcite{PODHAISKY2005409}:
 \begin{itemize}
 ...
 \end{itemize}
 \only<2->{
 Parallel two-step methods---in which the integrator stages can be evaluated simultaneously---exist:
 \begin{itemize}
  \item These would be very suitable for SIMD-acceleration \textrightarrow very simple deep vectorization.
 \end{itemize}
 }
\end{frame}
