...
\begin{frame}
  ...
  \begin{itemize}
   ...
   \item Method coefficients did not depend on the stepsize ratiom---no need to solve non-linear equations at each step---important for small problems where this cost could be prohibitive
   \item Superior performance for the developed method---compared to a single-step, fourth-order Rosenbrock solver---for very stiff problems\slash~stringent tolerances without Jacobian reuse.
  \end{itemize}
...
\begin{frame}
 ...
 \only<1>{
 Two-step methods avoid order-reduction for very stiff problems and due to use of inexact Jacobian~\footfullcite{PODHAISKY2005409}
 \begin{itemize}
