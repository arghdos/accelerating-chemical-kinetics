...
%opening
\title{Accelerating chemical kinetic integration for reactive flows on hybrid architectures: A Literature Review}
\author{Nick Curtis}
...
\begin{frame}
 ...
 \end{itemize}
 We will focus on \textbf{vectorization}, \textbf{improved integration} and \textbf{high performance computing} techniques.
\end{frame}
...
\setcounter{dummynote1}{\value{footnote}}
\addtocounter{dummynote1}{1}
 While most integration techniques may benefit from a deep SIMD vectorization (e.g. in LU factorization, matrix multiplies, etc.), complicated solvers may be poorly suited to a wide SIMD\slash SIMT vectorization.
 \begin{itemize}
  \item On some architectures (e.g. GPUs), a wide-vectorized solver may outperform a deep-vectorized version of the same\footnotemark[\value{dummynote1}].
  \item However, solvers with many conditional statements and program branches (e.g. implicit solvers) may suffer from increased thread-divergence\footnotemark[\value{dummynote1}].
 \end{itemize}
 It is therefore important to identify stiff solvers suitable for efficient wide-vectorization.
...
\begin{frame}
 ...
 \begin{itemize}
  ...
  \item \textbf{Less thread divergence} for wide-SIMD\slash SIMT implementations!
 \end{itemize}
 ...
 \end{itemize}
 \end{itemize}
 However, we may want to improve the performance...
\end{frame}

\begin{frame}
 \RosenTitle
 An s-stage Rosenbrock method is described as~\footfullcite{yu2009}:
 \begin{align}
  (I - h\gamma_{ii}\mathcal{J})k_{i}^{n} = h f(y^n + \sum_{j=1}^{i-1}{\alpha_{ij}k_{j}^{n}}) + h\mathcal{J}\sum_{j=1}^{i-1}{\gamma_{ij}k^{j}_{n}},&\; i = 1, \ldots, s \nonumber \\
  y^{n+1} = y^{n} + \sum_{j=1}^{s}{b_jk_j}&
 \end{align}
 where $\alpha_{ij}$, $\gamma_{ij}$ and $b_{ij}$ are method coefficients, and $h$, $f(\cdot)$, $\mathcal{J}$ are the step-size, chemical source term and resulting Jacobian, respectively.
 \begin{itemize}
   \item Let $\gamma_{ii} = \gamma = \textrm{const.}, \forall i \rightarrow $ \textit{singly implicit}
   \begin{itemize}
    \item Only one LU factorization per step (instead of one per stage)!
   \end{itemize}
 \end{itemize}
 Can we do better?
\end{frame}
...
\begin{frame}
 \TwoStepTitle
 \setcounter{dummynote1}{\value{footnote}}
 One outstanding issue, the so-called single-step Rosenbrock-type methods that we have looked at thus far:
 \begin{itemize}
...
\begin{frame}
 \TwoStepTitle
 \setcounter{dummynote1}{\value{footnote}}
 In peer methods, such as developed by Schmitt and Weiner~\footnotemark[\value{dummynote1}], several approximations to the solution---each with similar characteristics---exist at each step.
 \begin{itemize}
   ...
   \begin{itemize}
    \item A cost of $\mathcal{O}(s^3)$, negligible for large systems (potentially adverse for small systems)
   \end{itemize}
 ...
 \end{itemize}
 \footnotetext{\fullcite{Schmitt:2005}}
 \setcounter{footnote}{\value{dummynote1}}
\end{frame}
...
\begin{frame}
 \BalanceTitle
 In addition to accelerating chemical kinetic integration, \textit{load-balancing} is an imporant issue for speeding up reacting-flow simulations:
 \begin{itemize}
  \item Typically, the domain of a simulation is split up into smaller groupings of computational cells that may be assigned to different server nodes using MPI and solved in parallel
  \item If some nodes take much longer to solve than others, many server nodes will sit idle waiting on this bottleneck
 \end{itemize}
\end{frame}
\begin{frame}
 \BalanceTitle
 Kodavasal et al.~\footfullcite{kodavasal2016development} recently proposed a stiffness-based algorithm algorithm for chemistry load balancing in realistic reacting-flow simulations.
 \begin{itemize}
  \item A stiffness metric---wall clock timing of or number of internal integration steps taken during chemical kinetic integration---was calculated each computational after each CFD-timestep.
  \item The global average stiffness computed over all cells, and each MPI node transferred or recieved computational cells to attempt to match their stiffness to this average
 \end{itemize}
 Chemistry load balancing---measured by the ratio of longest to shortest chemical kinetic integration time per cell---was significantly improved, resulting in:
 \begin{itemize}
  \item A speedup of \SIrange{2.4}{3.2}{\times} over the unmodified load-balancing algorithm
...
\begin{frame}
 ...
 \end{itemize}
 Load balancing selected appropriate integration algorithms and number of computational cells based on the available hardware resources and chemical stiffness would likely provide great benefits for accelerating reacting-flow simulations.
 \begin{itemize}
