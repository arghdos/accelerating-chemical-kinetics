...
\begin{frame}
 \frametitle{Chemical Kinetic Integration}
 Stone and Niemeyer~\footfullcite{Stone:2016} investigated the effect of both wide and deep SIMD\slash SIMT parallelization of two explicit integration techniques (and accompanying chemical kinetic rate evaluation subroutines) on the CPU, GPU and an Intel Xeon Phi co-processor (MIC).
 \begin{itemize}
  ...
  \item Speedups of up to $1.4\times$, $2.5\times$ and $3.2\times$ (over a baseline CPU OpenMP version) were found for a stiff fourth-order-explicit Rosenbrock solver for the GPU, CPU, and MIC respectively.
