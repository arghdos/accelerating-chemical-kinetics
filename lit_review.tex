...
\begin{frame}
 ...
 \begin{itemize}
  \item On some architectures (e.g. GPUs), a wide-vectorized solver may outperform a deep-vectorized version of the same~\footfullcite{Stone:2013aa}.
  \addtocounter{footnote}{-1}
  \item However, solvers with many conditional statements and program branches (e.g. implicit solvers) may suffer from increased thread-divergence~\footfullcite{Stone:2013aa}.
 \end{itemize}
...
\begin{frame}
 ...
 \begin{itemize}
   \item Let $\gamma_{ii} = \gamma = \textrm{const.}, \forall i$
   \begin{itemize}
...
Several works have studied so-called parallel two-step W-Methods~\footfullcite{Jackiewicz2004389}$^,$\footfullcite{Schmitt:2004}$^,$\footfullcite{Schmitt:2005}, which are similar to Rosenbrock solvers.
\begin{itemize}
 \only<1>
 {
 \item These are \textit{linearly implicit}---they have an implicit formulation, but do not solve a set of nonlinear equations (no Newton iteration)
 \item These methods are \textit{parallel by design}---each stage can be computed in parallel (e.g. OpenMP, \textit{SIMD})
 \item W-Methods are similar to Rosenbrock solvers, but may utilize an approximate Jacobian
 \begin{itemize}
  \item Enables Jacobian\slash LU factorization reuse to save computation as in implicit techniques
 \end{itemize}
 \item These methods are stiffly accurate, but accuracy may be dependent on step-size ratios for stiff problems
 \begin{itemize}
  \item Step-size growth may need to be limited explicitly
 \end{itemize}
 }
 \only<2>
 {
 \item No order reduction due to high stage orders
 \item Storage of stage approximations from previous time-step are required
 }
\end{itemize}
\end{frame}
