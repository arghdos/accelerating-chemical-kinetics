...
\begin{frame}
 ...
 \only<2>
 {
  ...
  \begin{itemize}
    \item Instead of each vector-lane\footnote{Note: This work used a GPU, hence in a typical shallow-vectorization each GPU-thread solves an IVP; however the idea also applies to SIMD vectorization, so we will use ``vector-lane''} solving a single IVP, it was assigned multiple IVPs
...
\setcounter{dummynote1}{\value{footnote}}
\addtocounter{dummynote1}{1}
 While most integration techniques may easily benefit from deep SIMD vectorization (e.g., via vectorized LU factorization, matrix multiplication, etc.), complicated solvers may be poorly suited to a shallow SIMD\slash SIMT vectorization:
 \begin{itemize}
